...
float4 OceanSurfacePS(PixelInput input) : SV_TARGET
{
	...
	float2 perlin_tc = input.bumpMapSamplingPos2 / 20;// *g_PerlinSize + g_UVBase;
	//float2 perlin_tc = input.bumpMapSamplingPos1 * g_PerlinSize;
	...
	float2 perlin_0 = g_texPerlin.Sample(anisoSampler, perlin_tc0).xy;
	float2 perlin_1 = g_texPerlin.Sample(anisoSampler, perlin_tc1).xy;
	float2 perlin_2 = g_texPerlin.Sample(anisoSampler, perlin_tc2).xy;
	...
	float2 fft_tc = input.bumpMapSamplingPos1 / 20.0f;// input.uv; //(blend_factor > 0) ? input.uv : 0;
	
	float2 grad = g_texGradient.Sample(anisoSampler, fft_tc).xy;
	grad = lerp(perlin, grad, blend_factor);
	float3 normalPerlin = normalize(float3(grad, g_TexelLength_x2));
	// Reflected ray
	...
	float4 ramp = g_FresnelMap.Sample(pointSampler, cos_angle).xyzw;
	...
	{
		ramp = lerp(ramp, bend_param.z, (bend_param.x - reflect_vec.z) / (bend_param.x - bend_param.y));
	}
	
	reflect_vec.z = max(0, reflect_vec.z);
	
	float3 reflectionP = g_texReflectCube.Sample(linearSampler, reflect_vec).xyz;
	
	// Hack bit: making higher contrast
	reflectionP = reflectionP * reflectionP * 2.0f;
