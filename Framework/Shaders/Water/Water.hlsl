...
// -Updated on 5/28/2015 by Dustin Watson (Added Flow Map support)
//==============================================================================
...
WaterFlowPixelInput WaterFlowVS(WaterVertexInput input)
{
...
WaterNormalPixelInput WaterNormalVS(WaterVertexInput input)
{
...
float4 WaterNormalPS(WaterNormalPixelInput input) : SV_Target
{
	...
	//float fogLerp = saturate( (g_CamPos - 0.1) / 1000.0 );
	// Blend the fog color and the lit color.
	//color = lerp(color, float4(1, 1, 1, 1), fogLerp);
	
	//if( gFogEnabled == 1 )
	//{
	//	float fogLerp = saturate( (gCamPos - gFogNear) / gFogFar );
	//
		// Blend the fog color and the lit color.
...
float4 WaterFlowPS(WaterFlowPixelInput input) : SV_Target
{
	float4 color;
...
float4 WaterNormalWireframePS(WaterNormalPixelInput input) : SV_Target
{
    return float4(0.9f, 0.9f, 0.9f, 1);
}

float4 WaterFlowWireframePS(WaterFlowPixelInput input) : SV_Target
{
    return float4(0.9f, 0.9f, 0.9f, 1);
}