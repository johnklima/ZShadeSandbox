...
PixelInput LakeBedGBufferVS(VertexInput input)
{
	PixelInput output;
...
PixelOutput LakeBedGBufferPS(PixelInput input) : SV_Target
{
	float4 textureColor;
...
PixelOutput LakeBedGBufferWireframePS(PixelInput input) : SV_Target
{
	clip( input.clip < 0.0 ? -1 : 1 );
