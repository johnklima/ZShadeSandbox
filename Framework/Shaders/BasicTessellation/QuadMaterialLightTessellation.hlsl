//==============================================================================
// Quad Material Light Tessellation Shader
//
//==============================================================================
// History
//
...
HullInput QuadMaterialLightTessellationVS(VertexInput input)
{
	HullInput output;
...
HullInput QuadMaterialLightTessellationInstanceVS(VertexInputInstance input, uint instanceID : SV_InstanceID)
{
	HullInput output;
...
DomainInput QuadMaterialLightTessellationHS(InputPatch<HullInput, 4> inputPatch, uint pointID : SV_OutputControlPointID, uint patchID : SV_PrimitiveID)
{
	DomainInput output;
...
[domain("quad")]
PixelInput QuadMaterialLightTessellationDS(PatchConstOutput input, float2 uv : SV_DomainLocation, const OutputPatch<DomainInput, 4> inputPatch)
{
	PixelInput output;
...
float4 QuadMaterialLightTessellationPS(PixelInput input) : SV_Target
{
	float4 finalColor;
...
float4 QuadMaterialLightTessellationWireframePS(PixelInput input) : SV_Target
{
	clip(input.clip < 0.0 ? -1 : 1);
