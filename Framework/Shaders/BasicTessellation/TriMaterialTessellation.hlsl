...
HullInput TriMaterialTessellationVS(VertexInput input)
{
	HullInput output;
...
float4 TriMaterialTessellationPS(PixelInput input) : SV_Target
{
	float4 finalColor;
...
float4 TriMaterialTessellationWireframePS(PixelInput input) : SV_Target
{
	clip(input.clip < 0.0 ? -1 : 1);
