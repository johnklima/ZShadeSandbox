...
void BSPPVSGenerator::CalculatePVS(long sourceLeaf, ZShadeSandboxBSP::BSPPortal* srcPortal, ZShadeSandboxBSP::BSPPortal* targetPortal, long targetLeaf, BYTE* leafPVS)
{
	...
	ZShadeSandboxMath::XMMath3 sourceBoxMax(arrayData->leafArray[sourceLeaf].boundingBox.vMax);
	ZShadeSandboxMath::XMMath3 sourceBoxMin(arrayData->leafArray[sourceLeaf].boundingBox.vMin);

	ZShadeSandboxMath::XMMath3 targetBoxMax(arrayData->leafArray[targetLeaf].boundingBox.vMax);
	ZShadeSandboxMath::XMMath3 targetBoxMin(arrayData->leafArray[targetLeaf].boundingBox.vMin);

	// Find which side of the source portal the source leaf is on and which side of the target portal the target leaf is on
	ZShadeSandboxMath::XMMath3 sourceLeafCenter = (sourceBoxMax + sourceBoxMin) * 0.5f;
	ZShadeSandboxMath::XMMath3 targetLeafCenter = (targetBoxMax + targetBoxMin) * 0.5f;
