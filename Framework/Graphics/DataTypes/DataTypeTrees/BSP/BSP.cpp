...
void BSP::Load(string levelFilename)
{
	...
	BSPArrayData::Instance()->bytesPerSet = (BSPArrayData::Instance()->lNumberOfLeaves + 7) >> 3; // divide by 8
	BSPArrayData::Instance()->pvsData = (BYTE*)malloc(BSPArrayData::Instance()->lNumberOfLeaves * BSPArrayData::Instance()->bytesPerSet);
	ZeroMemory(BSPArrayData::Instance()->pvsData, sizeof(BSPArrayData::Instance()->lNumberOfLeaves * BSPArrayData::Instance()->bytesPerSet));
...
void BSP::Build(long node, ZShadeSandboxBSP::BSPPolygon* polygonList)
{
	...
	arrayData->nodeArray[node].planeID = LocateSplitter(polygonList);
	...
	if (count == 0)
	{
		...
		arrayData->leafArray[arrayData->lNumberOfLeaves].startPolygon = arrayData->lNumberOfPolygons;
		
		while (iter != 0)
		{
			arrayData->polygonArray[arrayData->lNumberOfPolygons] = *iter;
